...
export function DashboardSkeleton() {
  return (
    <div className="space-y-5">
      <section className="surface-elevated px-6 py-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        {Array.from({ length: 3 }).map((_, index) => (
          <article key={index} className="surface px-5 py-5">
          ...
          <div className="flex gap-3">
            {Array.from({ length: 7 }).map((_, index) => (
              <div key={index} className="flex-1 space-y-2">
                <div className="mx-auto h-32 w-full rounded-[12px] bg-white/10" />
                <div className="mx-auto h-3 w-8 rounded-full bg-white/10" />
              </div>
            ))}
          </div>
        </div>
